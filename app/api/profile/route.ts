...
export async function GET(req: Request) {
  try {
    ...
    // Optimized: Get count of completed questions without fetching all records
    const completedQuestions = await UserQuestionModel.countDocuments({ userId: user._id });

    // Optimized: Calculate rank using countDocuments instead of fetching all users
    const userRank = await UserSchema.countDocuments({ totalScore: { $gt: user.totalScore || 0 } }) + 1;
